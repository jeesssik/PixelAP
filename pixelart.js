			...
			fns:{
				...
				reSize: function() {
			        $('#sizeit').on('click', function(){
			        	var newSizeVal = $('#resize').val();
			        	if(newSizeVal==0 || newSizeVal > 50){
			        		alert("Ingresá un número entre 1 y 49");
			        		var newSizeVal=defaultCells;
			            	}
			            	else if(isNaN(newSizeVal)){
			            		alert("Ingresá sólo números");
			            		var newSizeVal=defaultCells;
			               	}
			               	myPixelDraw.fns.calcSize(newSizeVal);
			        });
			      },
			    ...
			    colorPalette: function() {
			        $('#color-pick > *').each(function(a,b) {
		            var clase = $(b).attr('class');
		            $(b).css('background-color', clase);
       				 });
			      },
			    pickColor: function() {
